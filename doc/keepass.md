...
The following new options are available for cpacoind and cpacoin-qt:
...
The following rpc commands are available:
 ...
 - _keepass init_: sets up the association between cpacoind and keepass by generating an AES key and sending an association message to KeePassHttp. This will trigger KeePass to ask for an Id for the association. Returns the association and the base64 encoded string for the AES key.
...
Sample initialization flow from _cpacoin-qt_ console (this needs to be done only once to set up the association):

 - Have KeePass running with an open database
 - Start _cpacoin-qt_
 - Open console
 - Type "_keepass init_" in cpacoin-qt console
 - Keepass pops up and asks for an association id, fill that in, for example, "_mydrkwallet_"
 - You should get a response like this "_Association successful. Id: mydrkwalletcpacoin - Key: AgQkcs6cI7v9tlSYKjG/+s8wJrGALHl3jLosJpPLzUE=_"
 - Edit _cpacoin.conf_ and fill in these values
```
keepass=1
...
Extended guide with screenshots is also available: https://cpacointalk.org/threads/keepass-integration.3620/
