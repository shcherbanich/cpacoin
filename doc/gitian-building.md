...
*Setup instructions for a Gitian build of Cpacoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Cpacoin
...
- [Building Cpacoin Core](#building-cpacoin-core)
...
Clone the git repositories for Cpacoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/cpacoinpay/cpacoin
```
...
in the Cpacoin Core repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Cpacoin Core
----------------

To build Cpacoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Cpacoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cpacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/cpacoinpay/cpacoin
...
For example:
```bash
URL=https://github.com/crowning-/cpacoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit cpacoin=${COMMIT} --url cpacoin=${URL} ../cpacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cpacoin=${COMMIT} --url cpacoin=${URL} ../cpacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cpacoin=${COMMIT} --url cpacoin=${URL} ../cpacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the cpacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../cpacoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/cpacoin.git
SIGPATH=/some/root/path/cpacoin-detached-sigs.git

./bin/gbuild --url cpacoin=${BTCPATH},signature=${SIGPATH} ../cpacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cpacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cpacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/cpacoin-osx-build.assert
```
...
[cpacoin/gitian.sigs](https://github.com/cpacoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
