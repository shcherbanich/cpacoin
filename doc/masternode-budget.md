...
Cpacoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ CPACOIN, _12_ times over the course of a year totaling _24000_ CPACOIN.

**Warning: if you change any fields within this command, the collateral transaction will become invalid.**

Format: ```mngovernance prepare proposal-name url payment-count block-start cpacoin-address monthly-payment-cpacoin```
...
Format: ```mngovernance submit proposal-name url payment-count block-start cpacoin-address monthly-payment-cpacoin fee-tx```
...
When block ```1000000``` is reached you'll receive a payment for ```1200``` CPACOIN to ```y6R9oN12KnB9zydzTLc3LikD9cCjjQzYG7```.
...
The following RPC commands are supported:
 ...
 - mngovernance "command"... ( "passphrase" )
  ...
  - vote               - Vote on a proposal by single masternode (using cpacoin.conf setup)
