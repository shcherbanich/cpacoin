...
const char * const BITCOIN_CONF_FILENAME = "cpacoin.conf";
const char * const BITCOIN_PID_FILENAME = "cpacoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "cpacoin" is a composite category enabling all Cpacoin-related debug output
            if(ptrCategory->count(string("cpacoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CpacoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CpacoinCore
    // Mac: ~/Library/Application Support/CpacoinCore
    // Unix: ~/.cpacoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CpacoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CpacoinCore";
#else
    // Unix
    return pathRet / ".cpacoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty cpacoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override cpacoin.conf
